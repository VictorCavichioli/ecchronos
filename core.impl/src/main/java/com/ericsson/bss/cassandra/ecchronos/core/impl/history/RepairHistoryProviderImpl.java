 ...
 */
package com.ericsson.bss.cassandra.ecchronos.core.impl.history;
...
import com.ericsson.bss.cassandra.ecchronos.core.repair.config.RepairEntry;
    ...
    class RepairEntryIterator extends AbstractIterator<RepairEntry>
    {
        ...
        protected RepairEntry computeNext()
        {
            while (myIterator.hasNext())
            {
                ...
                {
                    LongTokenRange tokenRange = extractTokenRange(row);
                    ...
                    long finishedAt = extractFinishedAt(row);
        ...
        private LongTokenRange extractTokenRange(Row row)
        {
        ...
        private Set<DriverNode> resolveNodes(Row row)
        {
            Set<InetAddress> participants = row.getSet(PARTICIPANTS_COLUMN, InetAddress.class);
            Set<DriverNode> nodes = new HashSet<>();

            // Resolve coordinator
            resolveAndAddNode(row.get(COORDINATOR_COLUMN, InetAddress.class), nodes, "Coordinator");

            // Resolve participants
            for (InetAddress participant : participants) {
                resolveAndAddNode(participant, nodes, "Participant");
            }

            return nodes;
        }

        private void resolveAndAddNode(InetAddress address, Set<DriverNode> nodes, String role)
        {
            Optional<DriverNode> node = myNodeResolver.fromIp(address);
            if (!node.isPresent()) {
                LOG.warn("{} node {} not found in metadata", role, address);
            } else {
                nodes.add(node.get());
            }
        }

        private long extractFinishedAt(Row row)
        {
