...
import com.ericsson.bss.cassandra.ecchronos.core.impl.jmx.ThrottlingLogger;
...
public final class HostStatesImpl implements HostStates, Closeable
{
    ...
    @Override
    public boolean isUp(final UUID nodeID, final InetAddress address)
    {
        refreshNodeStatus(nodeID);
    ...
    {
        return isUp(node.getHostId(), node.getBroadcastAddress().get().getAddress());
    }

    @Override
    public boolean isUp(final DriverNode node)
    {
        return isUp(node.getId(), node.getPublicAddress());
    }
